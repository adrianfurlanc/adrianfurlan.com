...
- Languages — English at `/`, Spanish at `/es/…` with the same page names. `astro.config.mjs` declares both locales, so `Astro.currentLocale` follows the address. All interface text lives in `src/i18n/ui.ts`: one object per language under identical keys, `es` typed as the shape of `en`, so a missing Spanish string fails `npx astro check`. To add text, add it to both sides and print `t.section.key` (`const t = ui[localeOf(Astro)]`). `src/i18n/index.ts` holds the helpers: `localeOf(Astro)` (the page's language), `localePath(locale, "/about")` for every internal link, `pathWithoutLocale` for the toggle, `dateLocale`. `src/i18n/player.ts` holds only the player's three labels because `PlayerBar.astro`'s client `<script>` imports it — never import `ui.ts` from a client script, it would ship every paragraph to the browser. The pages in `src/pages/es/` are thin wrappers that import the English page as a component (`es/blog/[slug].astro` and `es/blog/[...page].astro` also import and re-export their `getStaticPaths`); the English page renders in Spanish because the address starts with `/es/`. Sentences in `src/data/tracks.ts` are `{ en, es }` pairs. The post page's "N days ago" comes from `Intl.RelativeTimeFormat` with the document's `lang`.
